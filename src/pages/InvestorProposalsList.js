...
function InvestorProposalsList() {
      ...
      try {
        ...
        try {
          ...
          if (snapshot.docs.length > 0) {
            ...
            console.log(`Found ${snapshot.docs.length} investor proposal documents with simple query`);
            ...
            setError('No investor proposals found. Please add some investor proposals.');
          }
        } catch (fallbackErr) {
          ...
          setError('Unable to load investor proposals. Please try again later.');
        }
      } finally {
        setLoading(false);
      }
  ...
  const filteredProposals = selectedCategory === 'All Categories'
    ? proposals
    : proposals.filter(proposal => proposal.categories && proposal.categories.includes(selectedCategory));
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-8 rounded-lg text-center">
          <p className="text-lg">No investment opportunities found in this category.</p>
          <p className="mt-2">Try selecting a different category or add a new investment opportunity.</p>
          <button
            onClick={() => navigate('/investor-proposals/new')}
                ...
                <div className="border-t border-gray-100 pt-4 mt-4">
                  <div className="flex justify-between items-center text-sm">
                    <div className="text-gray-700">
                      ...
                      <p className="font-medium">₹{proposal.minAmount?.toLocaleString() || 0} - ₹{proposal.maxAmount?.toLocaleString() || 0}</p>
                    </div>
